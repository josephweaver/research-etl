...
def render_setup_script(
    *,
    ...
    chunk_venv_bootstrap.append("VENV_BASE=\"$VENV\"")
    ...
    chunk_venv_bootstrap.append("ETL_NODE_FAMILY=\"$(printf '%s' \"$ETL_SETUP_HOSTNAME\" | awk -F- '{print $1}')\"")
    ...
    chunk_venv_bootstrap.append("  if [ -d \"$VENV\" ]; then")
    chunk_venv_bootstrap.append("    case \"$VENV\" in")
    chunk_venv_bootstrap.append("      \"$ETL_REPO_ROOT\"/*) rm -rf \"$VENV\" ;;")
    chunk_venv_bootstrap.append("      *) echo \"[etl][setup] refusing to remove venv outside ETL_REPO_ROOT: $VENV\" >&2; exit 1 ;;")
    chunk_venv_bootstrap.append("    esac")
    chunk_venv_bootstrap.append("  fi")
    chunk_venv_bootstrap.append("  $PYTHON -m venv --copies \"$VENV\"")
    chunk_venv_bootstrap.append("}")
    chunk_venv_bootstrap.append("if [ ! -f \"$VENV/bin/activate\" ]; then")
    chunk_venv_bootstrap.append("  rebuild_venv")
    chunk_venv_bootstrap.append("fi")
    chunk_venv_bootstrap.append("if [ -x \"$VENV/bin/python\" ] && ! \"$VENV/bin/python\" -c 'import sys' >/dev/null 2>&1; then")
    chunk_venv_bootstrap.append("  echo \"[etl][setup] existing venv interpreter failed smoke test; rebuilding: $VENV\" >&2")
    chunk_venv_bootstrap.append("  rebuild_venv")
    chunk_venv_bootstrap.append("fi")
