        ...
        if self.db_tunnel_via_tmux:
            lines.extend(
                [
                    ...
                    "    echo '[hpcc_direct][db_tunnel] tmux tunnel session exited before port became ready' >&2",
                    "    break",
                    "  fi",
                    "  sleep 1",
                    "done",
                    "if [ \"$ETL_DB_TUNNEL_READY\" != \"1\" ]; then",
                    "  echo \"[hpcc_direct][db_tunnel] tunnel not ready at ${ETL_DB_TUNNEL_HOST:-127.0.0.1}:${ETL_DB_TUNNEL_PORT:-6543}\" >&2",
                    "  tmux capture-pane -pt \"$ETL_DB_TUNNEL_SESSION\" -S -200 >/tmp/hpcc_direct_db_tunnel_capture.$$ 2>/dev/null || true",
                    "  if [ -f /tmp/hpcc_direct_db_tunnel_capture.$$ ]; then cat /tmp/hpcc_direct_db_tunnel_capture.$$ >&2; rm -f /tmp/hpcc_direct_db_tunnel_capture.$$; fi",
                    "  exit 1",
                    "fi",
        ...
        for idx, asset in enumerate(asset_cache_specs):
            ...
            asset_cache_lines.extend(
                [
                    ...
                    "fi",
                    f"if [ -z \"$ASSET_COMMIT_{idx}\" ]; then echo '[hpcc_direct][assets] could not resolve commit for $ASSET_URL_{idx} ref=$ASSET_REF_{idx}' >&2; exit 1; fi",
                    f"ASSET_SHORT_SHA_{idx}=\"$(printf '%s' \"$ASSET_COMMIT_{idx}\" | cut -c1-12)\"",
                    f"ASSET_DIR_{idx}=\"$ASSET_CACHE_ROOT/${{ASSET_REPO_NAME_{idx}}}-${{ASSET_SHORT_SHA_{idx}}}\"",
                    ...
                    f"git -C \"$ASSET_DIR_{idx}\" checkout --detach \"$ASSET_COMMIT_{idx}\"",
                    f"git -C \"$ASSET_DIR_{idx}\" reset --hard \"$ASSET_COMMIT_{idx}\"",
