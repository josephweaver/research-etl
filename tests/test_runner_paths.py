...
def test_runner_creates_step_uuid_workdir_and_logs(tmp_path: Path) -> None:
    plugin_dir = tmp_path / "plugins"
    plugin_dir.mkdir(parents=True, exist_ok=True)
    (plugin_dir / "echo.py").write_text(
        "\n".join(
    ...
    assert result.steps and result.steps[0].step_id
    step_id = str(result.steps[0].step_id)
    assert re.fullmatch(r"[0-9a-f]{32}", step_id)
    assert (stamped_workdir / "echo_step" / step_id).is_dir()
    assert (stamped_workdir / "echo_step" / step_id / "logs" / "step.log").exists()
