...
def test_slurm_includes_db_tunnel_command_in_setup_and_batch_scripts(monkeypatch, tmp_path: Path) -> None:
    ...
    tunnel_cmd = "ssh -f -N -L 6543:db.host:5432 weave151@hpcc.msu.edu"
    ex = SlurmExecutor(
        {"workdir": "/tmp/work", "logdir": "/tmp/logs", "db_tunnel_command": tunnel_cmd},
        repo_root=tmp_path,
        plugins_dir=Path("plugins"),
        dry_run=False,
    )
    ...
    assert "rewrite_tunneled_database_url" in batch_script
...
def test_slurm_wraps_db_tunnel_with_tmux_and_cleanup_when_enabled(monkeypatch, tmp_path: Path) -> None:
    ...
    assert "ETL_DB_TUNNEL_READY=0" in setup_script
    assert "ETL_DB_TUNNEL_READY=0" in batch_script
...
def test_slurm_rewrites_pipeline_path_from_asset_cache_to_checkout_pipelines(monkeypatch, tmp_path: Path) -> None:
    ...
    asset_pipeline = repo_root / ".pipeline_assets_cache" / "shared-etl-pipelines-46263747e4" / "pipelines" / "testdb.yml"
    ex.submit(str(asset_pipeline), {"run_id": "runabc1234", "repo_root": repo_root})

    batch_script = calls[1]["script_text"]
    assert "/.pipeline_assets_cache/" not in batch_script
    assert "/pipelines/testdb.yml" in batch_script
    ...
    ex = SlurmExecutor(
        {
            "workdir": "/tmp/work",
            "logdir": "/tmp/logs",
            "database_url": "postgresql://cfg_user:cfg_pass@cfg_host:5432/cfg_db",
        },
        repo_root=tmp_path,
        plugins_dir=Path("plugins"),
        dry_run=True,
    )
    assert ex.database_url == "postgresql://cfg_user:cfg_pass@cfg_host:5432/cfg_db"
